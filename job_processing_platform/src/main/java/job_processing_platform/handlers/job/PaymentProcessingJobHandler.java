...
@Component
public class PaymentProcessingJobHandler extends AbstractJobHandler {

    @Override
    public JobHandlerType identify() {
        return JobHandlerType.PAYMENT;
    }
    ...
    protected void execute(JobMessage message) {
        if (Math.random() < 0.2) {
            throw new RuntimeException("Payment gateway rejected transaction");
        }
